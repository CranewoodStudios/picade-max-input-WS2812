#!/usr/bin/env python3
# pm_daemon.py — Event-driven LED daemon (FIFO version, Picade Max USB)
# Reads JSON lines from /tmp/pm.fifo and drives the Picade Max input board:
...
def pack_colors(cols):
    ...
    for (b, g, r, br) in cols:
        payload += bytes((_clamp(b), _clamp(g), _clamp(r), _clamp(br, hi=BRIGHT_LIMIT)))
    return HEADER + payload


def send_colors(ser, cols):
    mapped = [cols[src] if src < len(cols) else (0, 0, 0, 0) for src in ORDER]
    ser.write(pack_colors(mapped))
    ser.flush()
...
def solid(b, g, r, br): return [(b, g, r, _clamp(min(br, BRIGHT_LIMIT), hi=BRIGHT_LIMIT))] * NUM_LEDS
...
# ---------- frames ----------
def breath_frame(t, color=(0, 0, 255, 25), speed=1.2, minf=0.2, maxf=1.0):
    bb, bg, br, bbr = color
    f = (math.sin(t*speed)+1.0)/2.0
    f = minf + (maxf-minf)*f
    return [(bb, bg, br, _clamp(int(bbr*f), hi=BRIGHT_LIMIT)) for _ in range(NUM_LEDS)]


def wipe_frames(color=(0, 16, 16, BRIGHT_LIMIT), step_ms=50):
...
def fade_all(from_lvl=20, to_lvl=0, ms_total=700):
    steps = max(1, int(ms_total/20))
    for s in range(steps+1):
        lvl = _clamp(int(lerp(from_lvl, to_lvl, s/steps)), hi=BRIGHT_LIMIT)
...
def cols_from_layout(layout):
    ...
    for i in range(min(NUM_LEDS, len(layout))):
        ...
        if isinstance(item, dict):
            ...
            cols.append((b, g, r, _clamp(min(br, BRIGHT_LIMIT), hi=BRIGHT_LIMIT)))
...
def system_accent(system_key):
    ...
    if isinstance(c, dict):
        ...
        g = int(c.get("g", 0))
        r = int(c.get("r", 0))
        br = _clamp(min(int(c.get("br", 16)), BRIGHT_LIMIT), hi=BRIGHT_LIMIT)
        return (b, g, r, br)
    return None
...
def default_attract_mode():
    d = _cfg.get("defaults", {})
    return (d.get("attract") or "breath").lower()
...
def anim_game_start(ser, system_key=None, rom_key=None):
    ...
    if layout:
        ...
        time.sleep(1.0)
        return
    accent = system_accent(system_key) or (0, 16, 0, BRIGHT_LIMIT)
    for cols in wipe_frames(color=accent, step_ms=40):
        send_colors(ser, cols)
    send_colors(ser, solid(0, 0, 0, 8))
    time.sleep(0.25)


def anim_game_end(ser):
    for cols in wipe_frames(color=(16, 0, 0, BRIGHT_LIMIT), step_ms=40):
        send_colors(ser, cols)
    for cols in fade_all(from_lvl=12, to_lvl=6, ms_total=600):
        send_colors(ser, cols)


def anim_shutdown(ser):
    for _ in range(3):
        send_colors(ser, solid(0, 0, 0, BRIGHT_LIMIT))
        time.sleep(0.08)
        send_colors(ser, solid(0, 0, 0, 4))
        time.sleep(0.1)
    for cols in fade_all(from_lvl=12, to_lvl=0, ms_total=900):
        send_colors(ser, cols)


def anim_reboot(ser):
    for cols in wipe_frames(color=(0, 16, 0, BRIGHT_LIMIT), step_ms=35):
        send_colors(ser, cols)
    for cols in wipe_frames(color=(16, 0, 0, BRIGHT_LIMIT), step_ms=35):
        send_colors(ser, cols)
    for cols in fade_all(from_lvl=10, to_lvl=0, ms_total=500):
        send_colors(ser, cols)


def anim_settings_changed(ser):
    col = (8, 8, 0, BRIGHT_LIMIT)
    for _ in range(3):
        send_colors(ser, solid(*col))
        time.sleep(0.12)
        ...
        while True:
            ...
            for i in range(NUM_LEDS):
                ...
                if k == 0:
                    ...
                    rgb = (v, p, q)
                cols.append((rgb[0], rgb[1], rgb[2], 8))
            yield cols
    else:
        while True:
            yield breath_frame(time.monotonic()-t0, color=(0, 0, 0, 10), speed=0.6, minf=0.15, maxf=0.6)
...
def find_serial_port():
    ...
    try:
        for name in os.listdir(byid):
            low = name.lower()
            if ("picade" in low) or ("pimoroni" in low and "max" in low):
...
def main():
    ...
    if not port:
        log("ERROR: no serial port found (is the Picade Max connected & powered?)")
        return
    ser = serial.Serial(port, 115200, timeout=0.05)
    log("daemon started; PORT =", port, "FIFO =", FIFO_PATH, "LEDs =", NUM_LEDS)
